...
sample(diamonds$price, 50, replace = F)
...
pbinom(7,10, prob  =0.5, lower.tail = FALSE)
...
# /hidden d for distribution function
# /hidden p for the cumulative probability function
```
...
pnorm(q=66, mean = 70, sd = 3, lower.tail = FALSE)
...
cat("the 95% confidence interavl of a standard normal variable is from", qnorm(0.025), "to", qnorm(0.975))
```
